...
class DifuntosRPA {
    ...
    private async getValidateLive() {
        ...
        const nitsFormatted = initialNits.map(nit => `'${nit}'`).join(',');
        const solicitudes = await this.connection.query(`SELECT TOP 10 * FROM T_difuntos_RPA WHERE muerto IS NULL AND nit IN (${nitsFormatted})`);
        return solicitudes;
    }

    private getInitialNits(): string[] {    
        return [    
            '916836',
            '3715359',
            '4726655',
